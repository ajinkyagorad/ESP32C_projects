...
// Servo configuration
#define SERVO_PIN 1  // Using PIN D1 from OLED ESP32-C3 module
Servo servo;
...
<body>
    ...
    <div class="control">
        ...
        <!-- Added Servo Control Slider -->
        <h2>Servo Control</h2>
        <label>Servo Angle: </label>
        <input type="range" min="0" max="180" value="90" id="servo" onchange="sendServo(this.value)">
        <span id="servoValue">90</span><br><br>
    </div>
    ...
    <script>
        ...
        function sendServo(value) {
            sendMessage('SERVO ' + value); // Send the servo angle to the server
            document.getElementById('servoValue').innerText = value; // Update displayed value
        }
    </script>
...
void onWebSocketMessage(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
    if (type == WStype_TEXT) { // Only process text messages
        ...
        } else if (msg.startsWith("SERVO")) { // Handle servo control
            int servoAngle = msg.substring(6).toInt(); // Extract angle from message
            servo.write(servoAngle); // Move the servo to the specified angle
            webSocket.sendTXT(num, "Servo moved to " + String(servoAngle) + "°");
...
void setup() {
    ...
    // Initialize Servo
    servo.attach(SERVO_PIN, 500, 2500); // Pulse width range: 500us to 2500us
